...
import { UserPlusIcon, ListBulletIcon, Squares2x2Icon, SunIcon, MoonIcon } from './icons';
...
const Header: React.FC<HeaderProps> = ({ employeeCount, viewMode, theme, user, onAddEmployee, onSetViewMode, onToggleTheme, onLogout }) => {
    return (
        <header className="sticky top-0 z-10 bg-white dark:bg-slate-800 shadow-md">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-16">
                    ...
                    <h6>Qualquer erro ou dificuldade, só informar o Tio Cacheado</h6>
                    <div className="flex items-center space-x-2 sm:space-x-4">
                        ...
                        {user && (
                            <div className="flex items-center space-x-3">
                                ...
                                <button
                                    onClick={onLogout}
                                    className="hidden sm:inline bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-semibold py-2 px-3 rounded-md transition-colors text-sm"
                                >
                                    Sair
                                </button>
                            </div>
