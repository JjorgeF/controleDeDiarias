...
import { ChevronLeft, ChevronRight, Users, Search, UserPlus, UserMinus, Clock, Copy, ClipboardPaste, AlertTriangle, Maximize2, Lock, Unlock, Save, Calendar, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function CalendarView({ 
  ...
  const handleDayClick = (day: Date) => {
    if (isAdmin) {
      ...
      if (currentAvailabilities.includes(dayStr)) {
        newAvailabilities = currentAvailabilities.filter(d => d !== dayStr);
      } else {
        newAvailabilities = [...currentAvailabilities, dayStr];
      }
            ...
            <div className="overflow-x-auto sm:overflow-x-visible">
              ...
              <div className="grid grid-cols-7 auto-rows-fr min-w-[320px] md:min-w-[700px]">
                {calendarDays.map((day, idx) => {
                  ...
                  const availablesCount = employees.filter(emp => 
                    emp.availabilities?.includes(dayStr)
                  ).length;
                  
                  // Status for employee view
                  const isMyAvailable = myEmployee?.availabilities?.includes(dayStr);
                  const isMyScheduled = myEmployee?.workDays.some(d => d.date === dayStr);
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        "min-h-[50px] md:min-h-[100px] p-1 md:p-3 border-b border-r border-brand-border transition-all relative group",
                      ...
                      <div className="flex flex-col items-center justify-between h-full gap-1">
                        ...
                        {isAdmin ? (
                          <div className="flex flex-wrap gap-1 justify-center items-center mt-1">
                            ...
                            {availablesCount > 0 && (
                              <div 
                                title={`${availablesCount} disponíveis`}
                          ...
                          <div className="flex flex-col gap-1 items-center w-full mt-1">
                            {isMyScheduled && (
                              <div className="bg-brand-primary/20 text-brand-primary px-1 md:px-2 py-0.5 rounded text-[7px] md:text-[9px] font-black uppercase tracking-wider text-center shrink-0 w-full max-w-[54px] sm:max-w-none truncate">
                                <span className="hidden sm:inline">Escalado</span>
                                <span className="sm:hidden">Esc.</span>
                              </div>
                            )}
                            {isMyAvailable && !isMyScheduled && (
                              <div className="bg-emerald-500/20 text-emerald-400 px-1 md:px-2 py-0.5 rounded text-[7px] md:text-[9px] font-black uppercase tracking-wider text-center shrink-0 flex items-center justify-center gap-0.5 w-full max-w-[54px] sm:max-w-none truncate">
