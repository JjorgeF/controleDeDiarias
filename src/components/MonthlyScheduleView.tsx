...
type LayoutMode = 'grid' | 'matrix' | 'timeline';
...
export default function MonthlyScheduleView({
  ...
  // Returns map: dateStr -> Array of { employee, workDay }
  const dailyScheduleMap: Record<string, { employee: Employee; workDay: WorkDay }[]> = useMemo(() => {
      ...
      (emp.workDays || []).forEach(wd => {
        if (!wd.isCancelled && map[wd.date]) {
          ...
          const matchesSearch = !searchQuery || 
            ...
            (wd.partyName && wd.partyName.toLowerCase().includes(searchQuery.toLowerCase())) ||
            (wd.shift && wd.shift.toLowerCase().includes(searchQuery.toLowerCase()));

          if (matchesSearch) {
            // Check event type filter
            const matchesType = 
              eventFilter === 'all' ||
              (eventFilter === 'common' && wd.type === 'common') ||
              (eventFilter === 'party' && wd.type === 'party');
    ...
    // Sort employees inside each day by level / artisticName
    ...
    return map;
  }, [monthDays, employees, searchQuery, eventFilter, effectiveHighlightEmployee]);
  ...
  // Filtered days list if "onlyMyDays" is active
  const filteredMonthDays = useMemo(() => {
    if (!onlyMyDays || !effectiveHighlightEmployee) return displayedDays;
    return displayedDays.filter(day => {
      const dateStr = format(day, 'yyyy-MM-dd');
      const items = dailyScheduleMap[dateStr] || [];
      return items.some(item => item.employee.id === effectiveHighlightEmployee.id);
    });
  }, [displayedDays, onlyMyDays, effectiveHighlightEmployee, dailyScheduleMap]);
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center bg-brand-bg border border-brand-border rounded-xl p-1">
              <button
                ...
                className={cn(
                  ...
                  layoutMode === 'matrix' ? "bg-brand-primary text-slate-950 shadow-sm" : "text-gray-400 hover:text-white"
                )}
                title="Visão Matriz (Tabela)"
              >
                <Table size={14} />
                <span>Matriz</span>
              </button>
              ...
              >
                ...
                <div className={cn(
                  ...
                  <div className="flex items-center gap-2">
                    <div className={cn(
                      "w-10 h-10 rounded-xl flex flex-col items-center justify-center font-black shrink-0 border",
                      isCurrentToday
                      ...
                      <span className="text-[9px] uppercase font-extrabold leading-none mt-0.5 opacity-80">
                ...
                <div className="px-3 py-1.5 bg-brand-bg/40 border-t border-brand-border/40 text-[10px] font-bold text-gray-400 flex items-center justify-between">
                  ...
                  {isMyDay && myWorkItem && (
                    <span className="text-amber-300 font-extrabold flex items-center gap-1">
                      ...
                      <span>Seu turno: {myWorkItem.workDay.shift || myWorkItem.workDay.type}</span>
                    </span>
                  )}
                </div>
              </div>
      ...
      {layoutMode === 'matrix' && (
        <div className="bg-brand-card border border-brand-border rounded-2xl overflow-hidden shadow-xl">
          <div className="p-3.5 bg-brand-bg/60 border-b border-brand-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            <h3 className="text-xs font-black text-white uppercase tracking-wider flex items-center gap-2">
              <Table size={16} className="text-brand-primary" />
              <span>Matriz Unificada ({timeScope === 'month' ? 'Mês Inteiro' : `Semana ${timeScope.replace('week', '')}`})</span>
            </h3>
            <span className="text-[11px] text-gray-400">💡 Clique em qualquer dia ou célula para abrir os detalhes completos do dia</span>
          </div>

          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead>
                <tr className="bg-brand-bg/90 border-b border-brand-border text-[11px] font-black uppercase text-gray-400 select-none">
                  <th className="p-3 sticky left-0 z-20 bg-brand-bg border-r border-brand-border min-w-[190px]">
                    Recreador
                  </th>
                  {displayedDays.map(day => {
                    ...
                    const isWeekendDay = isWeekend(day);
                    const config = dayConfigs[dateStr];
                    const dayCount = (dailyScheduleMap[dateStr] || []).length;
                    ...
                    return (
                      <th 
                        ...
                        className={cn(
                          "p-2 text-center border-r border-brand-border/40 min-w-[85px] cursor-pointer transition-colors group relative hover:bg-brand-primary/20",
                          isCurrentToday ? "bg-brand-primary/20 text-brand-primary" : isWeekendDay ? "bg-purple-950/20 text-purple-300" : ""
                        )}
                        title={`Clique para ver detalhes do dia ${format(day, 'dd/MM')}`}
                      >
                        <div className="text-[10px] font-bold opacity-75">{format(day, 'eee', { locale: ptBR }).charAt(0).toUpperCase()}</div>
                        <div className="text-xs font-black text-white group-hover:text-brand-primary transition-colors">{format(day, 'd')}</div>
                        
                        {/* Event Indicator Dots */}
                        <div className="flex items-center justify-center gap-0.5 mt-0.5">
                          {config?.isCommon && <div className="w-1.5 h-1.5 rounded-full bg-brand-primary" title="CCSP" />}
                          {config?.parties && config.parties.length > 0 && <div className="w-1.5 h-1.5 rounded-full bg-purple-400" title="Festa" />}
                          {config?.isExtraordinaryOpen && <div className="w-1.5 h-1.5 rounded-full bg-amber-400" title="Abertura Extra" />}
                        </div>
                      </th>
                    );
                  })}
                  <th className="p-3 text-center min-w-[70px]">Total</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-brand-border/40 text-xs">
                ...
                {employees
                  .sort((a, b) => {
                    if (effectiveHighlightEmployee && a.id === effectiveHighlightEmployee.id) return -1;
                    if (effectiveHighlightEmployee && b.id === effectiveHighlightEmployee.id) return 1;
                    return (a.artisticName || a.name).localeCompare(b.artisticName || b.name, 'pt-BR');
                  })
                  .filter(emp => 
                    !searchQuery || 
                    emp.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                  ...
                  .map(emp => {
                    const isHighlighted = effectiveHighlightEmployee && emp.id === effectiveHighlightEmployee.id;

                    // Calculate workDays for displayed range
                    const empWorkDays = (emp.workDays || []).filter(wd => {
                      if (wd.isCancelled) return false;
                      const dateObj = parseISO(wd.date);
                      return displayedDays.some(d => isSameDay(d, dateObj));
                    });
                    ...
                    return (
                      ...
                      >
                        {/* Sticky Name Column */}
                        <td className={cn(
                          "p-2.5 sticky left-0 z-10 border-r border-brand-border flex items-center gap-2",
                          isHighlighted ? "bg-amber-950/90 text-amber-200" : "bg-brand-card group-hover:bg-brand-bg"
                        )}>
                          {emp.photoUrl ? (
                            <img src={emp.photoUrl} alt="" className="w-7 h-7 rounded-full object-cover shrink-0 border border-brand-border" />
                          ) : (
                            <div className="w-7 h-7 rounded-full bg-brand-primary/20 text-brand-primary text-[11px] font-black flex items-center justify-center shrink-0 border border-brand-primary/30">
                              {(emp.artisticName || emp.name).charAt(0)}
                            </div>
                          )}
                          <div className="truncate min-w-0">
                            <p className="truncate font-bold text-white flex items-center gap-1">
                              <span>{emp.artisticName || emp.name}</span>
                              {isHighlighted && (
                                <Star size={11} className="fill-amber-400 text-amber-400 shrink-0" />
                              )}
                            </p>
                            <p className="text-[9px] text-gray-400 truncate">{emp.level}</p>
                          </div>
                        </td>
                        ...
                        {displayedDays.map(day => {
                          const dateStr = format(day, 'yyyy-MM-dd');
                          const workDay = empWorkDays.find(wd => wd.date === dateStr);
                          const isCurrentToday = isToday(day);
                          ...
                          return (
                            <td 
                              ...
                              onClick={() => setSelectedDayModal(dateStr)}
                              className={cn(
                                "p-1 text-center border-r border-brand-border/30 relative cursor-pointer hover:bg-brand-primary/10 transition-colors",
                                isCurrentToday ? "bg-brand-primary/5" : ""
                              )}
                            >
                              {workDay ? (
                                <div 
                                  className={cn(
                                    "w-full min-h-[38px] py-1 px-1.5 rounded-lg flex flex-col items-center justify-center text-[9.5px] leading-tight font-black transition-all shadow-sm overflow-hidden text-center",
                                    workDay.type === 'party'
                                      ? "bg-purple-900/80 text-purple-200 border border-purple-500/50"
                                      : "bg-brand-primary/25 text-brand-primary border border-brand-primary/50"
                                  )}
                                  title={`${emp.artisticName || emp.name} - ${workDay.shift || workDay.type} (${format(day, 'dd/MM')}) - Clique para detalhes`}
                                >
                                  {workDay.type === 'party' ? (
                                    <span className="truncate max-w-full">🎉 {workDay.partyName || 'Festa'}</span>
                                  ) : workDay.shift ? (
                                    (() => {
                                      const match = workDay.shift.match(/^(.*?)\s*(\(.*?\))$/);
                                      if (match) {
                                        return (
                                          <>
                                            <span className="font-extrabold truncate max-w-full text-[9px]">{match[1]}</span>
                                            <span className="text-[8.5px] font-mono opacity-90 truncate max-w-full">{match[2]}</span>
                                          </>
                                        );
                                      }
                                      return <span className="break-words text-[9px] leading-tight max-w-full">{workDay.shift}</span>;
                                    })()
                                  ) : (
                                    <span>✓ CCSP</span>
                                  )}
                                ...
                                <div className="w-full h-9 flex items-center justify-center text-gray-700 text-[10px] hover:text-gray-400">
                                  •
                                </div>
                              )}
                            </td>
                        ...
                        {/* Total Count Cell */}
                        <td className="p-2 text-center font-black text-brand-primary bg-brand-bg/60">
                          {empWorkDays.length}
                        </td>
                      </tr>
                    );
              ...
              {/* Table Footer Totals */}
              <tfoot>
                <tr className="bg-brand-bg border-t-2 border-brand-border text-xs font-black text-white">
                  <td className="p-3 sticky left-0 z-20 bg-brand-bg border-r border-brand-border text-gray-300 uppercase tracking-wider text-[10px]">
                    Total de Recreadores
                  </td>
                  {displayedDays.map(day => {
                    const dateStr = format(day, 'yyyy-MM-dd');
                    const count = (dailyScheduleMap[dateStr] || []).length;
                    return (
                      <td 
                        key={dateStr}
                        onClick={() => setSelectedDayModal(dateStr)}
                        className="p-2 text-center border-r border-brand-border/40 font-mono text-xs cursor-pointer hover:bg-brand-primary/10 text-brand-primary"
                      >
                        {count > 0 ? count : '-'}
                      </td>
                    );
                  })}
                  <td className="p-2 text-center font-black text-amber-400 bg-brand-bg">
                    {displayedDays.reduce((acc, day) => acc + (dailyScheduleMap[format(day, 'yyyy-MM-dd')] || []).length, 0)}
                  </td>
                </tr>
              </tfoot>
            </table>
