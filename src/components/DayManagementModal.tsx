                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {availableMarked.map(emp => (
                    <div 
                      key={emp.id} 
                      onClick={() => toggleWorkDay(emp)}
                      className="flex items-center justify-between bg-emerald-950/20 border border-emerald-500/30 p-3 rounded-xl hover:border-emerald-500/60 hover:bg-emerald-500/10 transition-all cursor-pointer group"
                    >
                      <div>
                        <p className="text-sm font-bold text-white group-hover:text-emerald-400 transition-colors">{emp.artisticName || emp.name}</p>
                        <p className="text-[10px] text-emerald-400 font-bold uppercase">{emp.level}</p>
                      </div>
